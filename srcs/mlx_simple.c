...
int	main(void)
{
	...
	mlx_hook(vars.win, ON_MOUSEMOVE, 1L, mouse_move, &vars);
	mlx_hook(vars.win, ON_MOUSEDOWN, 1L, mouse_down, &vars);
	mlx_hook(vars.win, ON_MOUSEUP, 1L, mouse_up, &vars);
	mlx_key_hook(vars.win, key_hook, &vars);
	mlx_hook(vars.win, ON_DESTROY, 1L, window_close, &vars);
